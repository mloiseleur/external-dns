...
func TestNewFactoriesBuildsOnePerNamespace(t *testing.T) {
	tests := []struct {
		...
		{name: "one per namespace, deduplicated", namespaces: []string{"team-a", "team-b", "team-a"}, want: []string{"team-a", "team-b"}},
		{name: "cluster wide", namespaces: nil, want: []string{corev1.NamespaceAll}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := make([]string, 0, len(tt.want))
			for _, factory := range newStartableFactories(tt.namespaces).All() {
				got = append(got, factory.namespace)
			}
			assert.Equal(t, tt.want, got, "each factory must be built for its own namespace")
		})
	}
}
...
func TestWaitForCacheSyncAll(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, factory := range factories.All() {
				if tt.expectedErr != "" && factory.namespace == tt.unsynced {
