...
func testCRDSourceEndpoints(t *testing.T) {
	for _, ti := range []struct {
		...
		{
			// Without --default-targets, dedupSource would drop it with only a log line.
			title:           "crd with no targets is rejected without default-targets",
			namespaceFilter: "foo",
			objectNamespace: "foo",
			endpoints: []*endpoint.Endpoint{
				{
		...
		t.Run(ti.title, func(t *testing.T) {
			...
			cs, err := newCrdSource(t.Context(), fakeCache, fakeCache.Client, ti.namespaceFilter, ti.labelSelector, ti.annotationSelector, nil)
			require.NoError(t, err)
...
func TestCRDSourceIllegalTargetWarnings(t *testing.T) {
	for _, ti := range []struct {
		...
		{
			title: "CNAME with empty targets produces no warning under --default-targets",
			endpoints: []*endpoint.Endpoint{
				{
					DNSName:    "example.org",
					Targets:    endpoint.Targets{},
...
func TestCRDSource_Endpoints_ObservedGenerationUpdateFailure(t *testing.T) {
	...
	cs, err := newCrdSource(t.Context(), fakeCache, failWriter, "", nil, nil, nil)
	require.NoError(t, err)
...
func helperCreateWatcherWithInformer(t *testing.T) (*cachetesting.FakeControllerSource, *crdSource) {
	...
	cs, err := newCrdSource(ctx, fakeCache, fakeCache.Client, "", nil, nil, nil)
	require.NoError(t, err)
